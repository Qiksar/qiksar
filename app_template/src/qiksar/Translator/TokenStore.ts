...
export default class TokenStore {
    ...
    Expand(inputText:string): string {
        ...
        while(returnText && returnText !== previousText && previousText.indexOf('{{') != -1) 
        {
    ...
    private ReplaceTokens(inputText:string):string {
        ...
        const tokenPath = inputText.substring(start + 2, end).trim().toUpperCase();
        const tokenValue = this.GetTokenValue(tokenPath);
        const returnText = inputText.substring(0, start) + tokenValue + inputText.substring(end + 2);
